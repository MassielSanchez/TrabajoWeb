...
export function setupCarrerasGrid() {
    const carrerasGrid = document.querySelector('.carreras-grid');
    if (!carrerasGrid) {
        console.warn('Carreras grid element not found. Skipping carreras setup.');
        return;
    }
    ...
    carrerasGrid.addEventListener('click', (event) => {
        ...
        if (carreraSeleccionada === carreraElemento) {
            // Si se hace clic en la misma carrera seleccionada, ocultar información
            if (infoMostrada) {
                infoMostrada.remove();
                infoMostrada = null;
            }
            ...
            // Si se hace clic en una carrera diferente
            const info = infoCarreras[carreraId];
            if (info) {
                // Remover la información mostrada previamente, si existe
                if (infoMostrada) {
                    infoMostrada.remove();
                }

                // Crear y mostrar la nueva información
                const nuevoInfoDiv = crearInfoCarrera(info);
                // Insertar la información después del elemento de la carrera clicada
                ...
                // Opcional: Desplazarse a la vista de la información mostrada
                nuevoInfoDiv.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
            } else {
                // Si la carrera no tiene información definida, asegurarse de que no haya info visible
                if (infoMostrada) {
                    infoMostrada.remove();
                    infoMostrada = null;
                }
